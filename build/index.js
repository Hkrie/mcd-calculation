...
const calcAverageValueArray = (array, allowedLengthOfArrays) => {
    ...
    for (let i = 0; i < array[0].length; i++) {
        ...
        const average_value = ___namespace.sum(convertToAbsoluteValueArray(ion_currents_of_amu)) / ion_currents_of_amu.length;
...
function calcSensitivities(calibrationMixture, partialPressures, measurementData) {
    return ___namespace.flatten(calibrationMixture.map(obj => {
...
function getMoleculesWithUniqAmus(remainingMixture) {
    ...
    remainingMixture.forEach(obj => {
        ...
        if (!___namespace.isEmpty(uniq_amus)) {
            arr.push({
                symbol: obj.symbol,
                uniq_amus: uniq_amus
            });
...
function calcNormIonCurrents(resolvedIonCurrents, calibrationFactors) {
    return ___namespace.flatten(resolvedIonCurrents.map(obj => {
...
function calcIonCurrentsForMoleculeAmus(proportionObjectForUniq, measurementData, uniqMolecule, uniqIonCurrentsPerAmu) {
    return proportionObjectForUniq.amu_proportions.amus.map((amu, index) => {
        ...
        if (uniqMolecule.uniq_amus.includes(amu)) {
            const ion_current = ___namespace.flatten(uniqIonCurrentsPerAmu.filter(obj => obj.amu === amu))[0].ion_current;
            ...
            const base_ion_current = ___namespace.flatten(uniqIonCurrentsPerAmu.filter(obj => obj.amu === uniqMolecule.uniq_amus[0]))[0].ion_current;
...
function checkProportions(testGasMixture, proportions) {
    testGasMixture.map(obj => {
        ...
        if (___namespace.isEmpty(amus_of_substance)) {
...
function calcProportions(recipe, calibrationMixture, completeMeasurement) {
    ...
    return calibrationMixture.map(obj => {
        ...
        const hundred_percent_peak_ion_current = ___namespace.max(measurement_data_subset["ion_currents"]) || 0;
        return {
            symbol: obj.symbol,
            amu_proportions: {
...
function getResolveOrder(testGasMixture) {
    ...
    while (remaining_mixture.length) {
        ...
        uniq_molecule_amus.forEach(item => {
            ___namespace.remove(remaining_mixture, (obj) => {
...
function resolveIonCurrents(proportions, lastCompleteMeasurement, testGasMixture, recipe, resolveOrder) {
    ...
    resolveOrder.forEach(uniqMolecule => {
        const proportion_object_for_uniq = ___namespace.flatten(proportions.filter(obj => obj.symbol === uniqMolecule.symbol))[0];
        ...
        ion_currents_per_amu_per_molecule.push({
            ...
            total_ion_current: ___namespace.sum(all_ion_currents)
        });
    });
...
function calcConcentrations(testGasMixture, calibrationFactors, resolvedIonCurrents) {
    ...
    const sum_norm_ion_currents = ___namespace.sum(norm_ion_currents.map(obj => obj.normIonCurrent));
    ...
    const concentrations = testGasMixture.map(obj => {
        ...
        return {
            ...
            total_concentration: ___namespace.sum(concentrationList)
        };
    });
    return concentrations;
}

exports.calcCalibrationFactors = calcCalibrationFactors;
exports.calcConcentrations = calcConcentrations;
exports.calcPartialPressures = calcPartialPressures;
exports.calcProportions = calcProportions;
exports.getResolveOrder = getResolveOrder;
exports.resolveIonCurrents = resolveIonCurrents;
